...
const BALI_FACTS = [
  {
    ...
    borderColor: '#1e3a6e',
    title: 'Musim Hujan',
    subtitle: 'Periode Curah Hujan Tinggi',
  ...
  },
  {
    ...
    borderColor: '#3d2a00',
    title: 'Musim Kering',
    subtitle: 'Periode Curah Hujan Rendah',
  ...
  },
  {
    ...
    bg: '#160d2e',
    borderColor: '#3b1f6b',
    title: 'Variasi Elevasi',
    subtitle: 'Pengaruh Topografi Wilayah',
  ...
  },
  {
    ...
    subtitle: 'Distribusi Curah Hujan',
...
/** Mengembalikan tier warna berdasarkan nilai curah hujan (mm). */
...
/** Memformat tanggal ISO menjadi hari dalam bulan (angka). */
...
/** Mengarahkan peta ke koordinat wilayah yang dipilih. */
...
/** Tooltip custom untuk AreaChart curah hujan. */
...
/** Kartu statistik tunggal (rata-rata / tertinggi). */
...
/** Satu baris dropdown dengan label. */
...
/** Panel info + filter di bagian atas halaman (background putih). */
function TopPanel({ input, onInputChange, onFetch, loading, summary, tier }) {
  const filterFields = [
    {
      label: 'Wilayah', key: 'wilayah',
      options: WILAYAH_LIST.map(w => ({ value: w, label: w })),
    },
    {
      label: 'Tahun', key: 'tahun',
      options: TAHUN_LIST.map(y => ({ value: y, label: y })),
    },
    {
  ...
  return (
    <div style={{ background: '#ffffff', borderBottom: '1px solid #e2e8f0' }}>
      <div style={{ width: '100%', padding: '0 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '220px 190px 1fr', gap: 14 }}>
            ...
            {TIER_CONFIG.map(({ color, dot, label, range }) => (
              <div key={label} style={{
                ...
                <div style={{
                  ...
                  background: color, boxShadow: `0 0 0 3px ${dot}88`,
                  flexShrink: 0,
                }} />
                <span style={{
                  fontFamily: "'Sora', sans-serif",
...
/** Peta distribusi spasial wilayah terpilih. */
...
/** Grafik tren curah hujan harian. */
...
/** Bagian informasi iklim Bali: fakta + klimatogram. */
function InfoBali() {
  return (
    <section style={{
      background: '#ffffff',
      borderTop: '1px solid #0c2040',
...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res  = await fetch(`http://127.0.0.1:5000/api/rainfall?wilayah=${wilayah}&tahun=${tahun}&bulan=${bulan}`);
